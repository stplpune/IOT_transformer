...
export class UserRegisterComponent {
  ...
  // totalPages!: number;
  pageNumber: number = 1;
  displayedColumns = ['srNo', 'userName', 'substationName','address', 'action'];
  ...
  getAllUsers() {
    ...
    this.apiService.setHttp('GET', 'MSEB_iOT/api/User/GetAllUsers' + obj, false, false, false, 'baseUrl')
  ...
  deleteUser(obj:any){
    this.apiService.setHttp('DELETE', 'MSEB_iOT/api/User/DeleteUsers?Id=' + obj.userId, false, false, false, 'baseUrl');
    this.apiService.getHttp().subscribe({
      next: (res: any) => {
        if (res.statusCode == '200') {
